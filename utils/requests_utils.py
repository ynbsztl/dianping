    ...
    def get_requests(self, url, request_type):
        ...
        if request_type == 'proxy, no cookie':
            if self.ip_proxy:
                ...
                r = requests.get(url, headers=self.get_header(None, False), proxies=self.get_proxy(), timeout=20)
        ...
        if request_type == 'proxy, cookie':
            ...
            if self.ip_proxy:
                r = requests.get(url, headers=header, proxies=self.get_proxy(), timeout=20)
            else:
